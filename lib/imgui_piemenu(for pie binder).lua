...
local ImVec2,ImVec4,ImColor = imgui.ImVec2, imgui.ImVec4, imgui.ImColor
...
local function NewPieMenuContext(RadiusEmpty, RadiusMin, MinItemCount, MinItemCountPerLevel)
	local obj = {
		c_iMaxPieMenuStack = 0xff,
		c_iMaxPieItemCount = 0xff,
		...
		m_iLastFrame = 0,
		m_iMaxIndex = 0,
		m_oCenter = ImVec2(sw/2,sh/2),
		m_iMouseButton = 0,
		m_bClose = false,
		colors = {
			button = imgui.GetStyle().Colors[23],
			buttonHovered = imgui.GetStyle().Colors[24],
		},
	}
...
local function BeginPiePopup(menuCtx, pName, iMouseButton,diameter,radiusEmpty)
	...
	if imgui.IsPopupOpen(pName) then
		...
		menuCtx.radiusEmpty = radiusEmpty
		menuCtx.diameter = diameter
		...
		if bOpened then
			...
			if menuCtx.m_iLastFrame < (iCurrentFrame - 1) then
				menuCtx.m_oCenter = ImVec2(sw/2,sh/2)
			end
...
local function EndPiePopup(menuCtx)
	...
	local fCurrentRadius = menuCtx.radiusEmpty
	-- ImRect
	...
	for iIndex = 0, menuCtx.m_iMaxIndex do
		...
		local fRotate = fLastRotate - item_arc_span * ( oPieMenu.m_iCurrentIndex - 1 ) / oPieMenu.diameter
		local item_hovered = -1
		local fMaxRadius = fMinRadius + (fMenuHeight * oPieMenu.m_iCurrentIndex) / menuCtx.diameter--
		for item_n = 0, oPieMenu.m_iCurrentIndex - 1 do
			local text = oPieMenu.m_oItemNames[ item_n ]
			...
			local iColor = imgui.ColorConvertFloat4ToU32( hovered and menuCtx.colors.buttonHovered or menuCtx.colors.button )
			...
			pDrawList:AddText(text_pos, imgui.GetStyleColorU32(imgui.Col.Text), text.label)
			if hovered then
				item_hovered = item_n
				imgui.BeginTooltip()
				imgui.Text(text.desc)
				imgui.EndTooltip()
			end
		end
...
local function BeginPieMenu(menuCtx, pName,desc)
	...
	oPieMenu.m_fMaxItemSqrDiameter = fSqrDiameter --[[/ (menuCtx.diameter/3)--]]
	if fSqrDiameter > oPieMenu.m_fMaxItemSqrDiameter then
	end
	oPieMenu.m_oItemIsSubMenu[oPieMenu.m_iCurrentIndex] = true
	oPieMenu.m_oItemNames[oPieMenu.m_iCurrentIndex] = {label=pName,desc=desc}
...
local function PieMenuItem(menuCtx, pName, desc)
	assert(menuCtx.m_iCurrentIndex >= 0 and menuCtx.m_iCurrentIndex < menuCtx.c_iMaxPieItemCount)
	local oPieMenu = menuCtx.m_oPieMenuStack[menuCtx.m_iCurrentIndex]
	local oTextSize = imgui.CalcTextSize(pName, true)
	oPieMenu.m_oItemSizes[oPieMenu.m_iCurrentIndex] = oTextSize
...
local function New(...)
	...
	return {
		...
		BeginPiePopup = function(name, mouseButton,colors,diameter,radiusEmpty)
			menuContext.colors = colors or menuContext.colors
			return BeginPiePopup(menuContext, name, mouseButton,diameter,radiusEmpty)
		end,
		EndPiePopup = function()
			return EndPiePopup(menuContext)
		end,
		PieMenuItem = function(name, desc)
			return PieMenuItem(menuContext, name, desc)
		end,
		BeginPieMenu = function(name, desc)
			return BeginPieMenu(menuContext, name, desc)
		end,
		EndPieMenu = function()
...
defaultPieMenu.__VERSION = __VERSION
